...
// `order: "before"` (BPF_F_BEFORE) places us at the front of the interface's
// TCX chain — both so a passive observer sees packets before anything can
// drop/redirect them, and because on awsvpc task ENIs a datapath TCX egress
// program otherwise runs ahead of ours and our egress hook never sees the
// response traffic.
function startAttach(spec) {
  const ordered = { order: "before", ...spec };
  return new BpfObject(EXE)
    .bind("events", RINGBUF)
    .attach("on_ingress", ordered)
    .attach("on_egress", ordered)
    .start();
}
...
if (yeet.args.selftest) {
  ...
  subscribe((raw) => {
    ...
    let t = ""; for (let i = 0; i < Number(ev.captured); i++) { const c = d[i]; if (c === 0) break; t += String.fromCharCode(c); }
    if (ev.kind === 1) { // response
      resps++;
      const m = STATUS.exec(t);
      if (m) statusTally[m[1]] = (statusTally[m[1]] || 0) + 1;
      return;
    }
    reqs++;
  ...
  console.log(`[verify] events: ${reqs} requests, ${resps} responses`);
  const st = Object.entries(statusTally).sort((a, b) => b[1] - a[1]).map(([c, n]) => `${c}×${n}`).join("  ");
  console.log(`[verify] response status codes: ${st || "(none captured)"}`);
